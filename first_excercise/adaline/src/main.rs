...
fn main() {
    ...
    error_boundary_study(&data_entries);
}
...
fn learn_factor_study(data_entries: &Vec<DataEntry>) {
    ...
    let error_boundary: f32 = 0.22;
    let mut rng = rand::thread_rng();

    for learn_factor in [0.001, 0.002, 0.01, 0.02, 0.03, 0.05, 0.1] {
        let mut x_weights: Vec<f32> = vec![];
        let mut y_weights: Vec<f32> = vec![];
        let mut bias_weights: Vec<f32> = vec![];
        let mut reached_errors: Vec<f32> = vec![];
        ...
        println!(
            ...
            learn_factor, avg_epochs, avg_reached_error, avg_x_weight, avg_y_weight, avg_bias_weight
        );
    }
}

/// the third task
fn error_boundary_study(data_entries: &Vec<DataEntry>) {
    let weight_range = -0.1..0.1;
    let learn_factor = 0.02;
    let mut rng = rand::thread_rng();

    for error_boundary in [0.3, 0.25, 0.22, 0.21, 0.20, 0.19, 0.18] {
        let mut x_weights: Vec<f32> = vec![];
        let mut y_weights: Vec<f32> = vec![];
        let mut bias_weights: Vec<f32> = vec![];
        let mut reached_errors: Vec<f32> = vec![];
        ...
        println!(
            ...
            error_boundary, avg_epochs, avg_reached_error, avg_x_weight, avg_y_weight, avg_bias_weight
        );
    }
}
